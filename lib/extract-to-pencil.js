...
export async function extractToPencil(projectDir, projectInfo) {
  ...
  try {
    ...
    if (!mcpAvailable) {
      spinner.warn('Pencil MCP not available - creating placeholder');
      await createPencilPlaceholder(projectDir, projectInfo);
      
      console.log(chalk.yellow('\n  ⚠️  Pencil MCP not detected'));
      console.log(chalk.dim('     To enable automatic extraction:'));
      console.log(chalk.dim('     1. Install Pencil Dev extension in Cursor'));
      console.log(chalk.dim('     2. Configure MCP connection'));
      console.log(chalk.dim('     3. Re-run the extraction\n'));
    ...
    // Try to generate actual .pen file
    console.log(chalk.bold.cyan('\n🎨 Attempting to generate .pen file...\n'));
    const penGenerated = await generatePenFile(projectDir, projectInfo, components, tokens);
    
    if (!penGenerated) {
      console.log(chalk.dim('     💡 Tip: .pen file generation requires active Pencil MCP connection'));
      console.log(chalk.dim('     The extraction metadata can be used to manually create the file.\n'));
    }
...
async function checkPencilMCP() {
  try {
    // Check if Pencil MCP tools are available
    const mcpDir = path.join(process.env.HOME, '.cursor', 'projects', 
      'Users-maxwroblewski-Test-project', 'mcps', 'user-pencil');
