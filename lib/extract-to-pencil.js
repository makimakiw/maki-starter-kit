...
export async function extractToPencil(projectDir, projectInfo) {
  ...
  try {
    ...
    const penFileGenerated = await generatePenFile(projectDir, projectInfo, components, tokens);
    
    if (!penFileGenerated) {
      console.log(chalk.red('  ⚠️  Failed to generate CREATE-PENCIL-FILE.md'));
      console.log(chalk.dim('     Check the error message above for details.\n'));
      return false;
    }
